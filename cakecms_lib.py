...
def send_email(subject, main_info, all_info=''):
    ...
    configuration.api_key['api-key'] = cakecms['mail']['api_key']
    api_instance = sib_api_v3_sdk.TransactionalEmailsApi(sib_api_v3_sdk.ApiClient(configuration))
    html_content = f'<html><body>'
    html_content += main_info
    html_content += all_info
    html_content += '</body></html>'
    sender = cakecms['mail']['sender']
    to = [cakecms['mail']['receiver']]
    ...
    if len(list(differences_dict_changes)) > 0:
        ...
        for section in differences_dict_changes:
            ...
            for elem in differences_dict_changes[section]:
                ...
                main_info += (f'Link: <a href="{differences_dict_changes[section][elem]["url"]}">Download</a><br>'
                              f'<br>')
                count += 1
            main_info += '<br>'
        if count > 1:
